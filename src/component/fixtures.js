...
const Fixtures = () => {
    ...
    const [datePre2, setDatePre2] = useState(new Date(today - milisecondsperday * 2));
    const [dateNext1, setDateNext1] = useState(new Date(today - -milisecondsperday));
    const [dateNext2, setDateNext2] = useState(new Date(today - -milisecondsperday * 2));
    ...
    return (
        <div className='fixtures'>
            <div className='timeline'>
                <a style={{ textDecoration: 'none', color: butindex === 2 ? 'orange' : 'black' }} onClick={() => handleDate(2)} href="#">
                    <div className='day'>{weekdays[datePre2.getDay()]}</div>
                    <div className='month'>{datePre2.getDate()} {months[datePre2.getMonth()]}</div>

                </a>
                <a style={{ textDecoration: 'none', color: butindex === 1 ? 'orange' : 'black' }} onClick={() => handleDate(1)} href="#">
                    <div className='day'>{weekdays[datePre1.getDay()]}</div>
                    <div className='month'>{datePre1.getDate()} {months[datePre1.getMonth()]}</div>
                </a>
                <a style={{ textDecoration: 'none', color: butindex === 0 ? 'orange' : 'black' }} onClick={() => handleDate(0)} href='#'>
                    <div className='day'>{date === today ? 'TODAY' : weekdays[date.getDay()]}</div>
                    <div className='month'>{date.getDate()} {months[date.getMonth()]}</div>
                </a>
                <a style={{ textDecoration: 'none', color: butindex === -1 ? 'orange' : 'black' }} onClick={() => handleDate(-1)} href="#">
                    <div className='day'>{weekdays[dateNext1.getDay()]}</div>
                    <div className='month'>{dateNext1.getDate()} {months[dateNext1.getMonth()]}</div>
                </a>
                <a style={{ textDecoration: 'none', color: butindex === -2 ? 'orange' : 'black' }} onClick={() => handleDate(-2)} href="#">
                    <div className='day'>{weekdays[dateNext2.getDay()]}</div>
                    <div className='month'>{dateNext2.getDate()} {months[dateNext2.getMonth()]}</div>
                </a>



                <input value={`${(date.getFullYear())}-${padTo2(date.getMonth() + 1)}-${padTo2(date.getDay())}`} type='date' style={{ width: '16px', height: '20px' }} onChange={(event) => handleDateChange(event)}></input>
